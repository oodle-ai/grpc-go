...
create grpc
[`credentials.TransportCredentials`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials#TransportCredentials)
base on TLS. Refer to the
[godoc](https://godoc.org/github.com/oodle-ai/grpc-go/credentials) for details.
...
[`credentials.NewServerTLSFromFile`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials#NewServerTLSFromFile).

On client side, we provide the path to the "ca_cert.pem" to configure TLS and create
the client credential using
[`credentials.NewClientTLSFromFile`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials#NewClientTLSFromFile).
...
[`grpc.Creds`](https://godoc.org/github.com/oodle-ai/grpc-go#Creds)) and let client dial
to the server with the created client credential (by calling
[`grpc.WithTransportCredentials`](https://godoc.org/github.com/oodle-ai/grpc-go#WithTransportCredentials))
...
[`alts.DefaultServerOptions`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials/alts#DefaultServerOptions)
to get the configuration for alts and then provide the configuration to
[`alts.NewServerCreds`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials/alts#NewServerCreds)
to create the server credential based upon alts.

On client side, first call
[`alts.DefaultClientOptions`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials/alts#DefaultClientOptions)
to get the configuration for alts and then provide the configuration to
[`alts.NewClientCreds`](https://godoc.org/github.com/oodle-ai/grpc-go/credentials/alts#NewClientCreds)
